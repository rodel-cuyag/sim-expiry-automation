...
```bash
python main.py --mode eod --agent-id 1060                                             # today (PHT)
...
- `--start-date` / `--end-date`: inclusive range in `YYYY-MM-DD`. Must be
  given together, or both omitted — omitting both defaults to **today
  (PHT)**, and the run stops with "No calls found ... Nothing to report" if
  there are no calls for today. Start cannot be after end.
